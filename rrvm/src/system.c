...
int system_attach_device_bus(bus_port_t * ports, int port_count, bus_t * bus) {
	for (int p = 0; p < port_count; p++) {
		bus_port_t * port = &ports[p];
		if (port->type == bus->type && *port->bus == NULL) {
			*port->bus = bus;
			return 0;
		}
	}
...
int system_attach_buses(system_t * system) {
	for (int i = 0; i < system->bus_count; i++) {
		if (system_attach_device_bus(system->cpu->ports, system->cpu->port_count, system->buses[i]) != 0) {
			return -1;
		}
	}
	for (int bus = 0; bus < system->bus_count; bus++) {
		for (int i = 0; i < system->device_count; i++) {
			device_t * device = system->devices[i];
			system_attach_device_bus(device->ports, device->port_count, system->buses[bus]);
		}
	}
	return 0;
}

int system_init_devices(system_t * system) {
	for (int i = 0; i < system->device_count; i++) {
		device_t * device = system->devices[i];
		device->init(device);
	}
...
void system_free(system_t * system) {
	...
	for (int i = 0; i < system->device_count; i++) {
		system->devices[i]->free(system->devices[i]);
	}
	free(system->buses);
	free(system->devices);
	free(system);
...
void system_clock(system_t * system) {
	...
	for (int i = 0; i < system->device_count; i++) {
		system->devices[i]->clock(system->devices[i]);
	}
}
...
system_t * system_create() {
	...
	if (system_init_devices(system) != 0) {
		printf("Could not init devices.\n");
		system_free(system);
		return NULL;
	}
